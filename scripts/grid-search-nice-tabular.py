...
class Tabular(NiceTabularQLearner):
    def __init__(
        self,
        lookback,
        discount_rate,
...
def main():
    agents = [TitForTat, OmegaTFT, Borufsen]
    ...
    if results:
        ...
        df.to_csv(f"results/nice-tabular-{d}.csv")

    if best_agent is not None:
        np.savez_compressed(
            f"models/{d} ({best_score}).npz", q_table=best_agent[1]._q_table
        )
        with open(f"models/nice-tabular-{d} ({best_score}).txt", "w") as f:
